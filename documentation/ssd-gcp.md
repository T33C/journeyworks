...
The User requires DR via a "separate zone", but GCP best practice for DR is a separate _region_. Zones provide HA, Regions provide DR. This design proposes a **Multi-Zone Application in Primary Region** (High Availability) and a **warm standby in a Secondary Region** (Disaster Recovery).
...
_Estimates based on typical production usage (sustained use)._
...
Two non-production environments are required: **SIT** (System Integration Testing) and **UAT** (User Acceptance Testing). UAT is specified at production-equivalent levels to ensure realistic performance validation. SIT is right-sized for functional and integration testing at reduced cost.

#### 6.3.1 SIT Environment (~50% of Production)

SIT is used for integration testing and does not require production-grade throughput, HA, or DR. Resources are reduced accordingly.
...
#### 6.3.2 UAT Environment (Production-Equivalent)

UAT mirrors production specifications to ensure realistic validation of performance, data volumes, and LLM behaviour before go-live. DR is not provisioned for UAT.
...
#### 6.3.3 Non-Production Cost Summary
